...
let Postext = (prop) => {
  ...
  if (Object.values(query) != "") {
    url_ = `${hostname}/auth/user/post/?userid=${query.userid}&postid=${query.postid}`;
  }
  ...
  let submit = async (e) => {
    ...
    let dep = await fetch(
      `${hostname}/auth/post/edit/?userid=${query.userid}&postid=${query.postid}`,
      {
        mode: "cors",
        method: "post",
        headers: {
    ...
    <div style={{ width: "100%" }}>
      ...
      <form style={{ width: "100%", height: "fit-content" }}>
        ...
        <textarea
          ...
          name="post"
        ></textarea>
        <button style={{ width: "100px" }} type="submit" onClick={submit}>
          post
        </button>
        <div>
